"""
CNN版本的 Double DQN Agent

只修改了网络输入部分（MLP→CNN），其他逻辑与原版ddqn_agent.py完全一致。
"""
...
from .cnn_networks import CNNFeatureExtractor
...
class CNNQNetwork(nn.Module):
    ...
    只把特征提取从MLP换成CNN，其他与原版QNetwork一致。
    """
...
class CNN_DDQNAgent:
    ...
    训练逻辑、输出格式与原版ddqn_agent.py完全一致，只是网络换成CNN版本。
    """
    ...
    def __init__(self, env, network, buffer, n_iter=100000, batch_size=32):
        ...
        # Epsilon衰减策略（与原版ddqn一致：前30%快速衰减）
        self.threshold = Threshold(
            seq_length=int(n_iter * 0.3),
            start_epsilon=1.0, 
            end_epsilon=0.05,
        ...
        self.initialize()
        self.player = PlayerQ_CNN(env=env, render=False)
...
class PlayerQ_CNN:
    """CNN版本的Player，用于评估（与原版PlayerQ接口一致）"""
