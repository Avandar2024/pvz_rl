...
def sum_onehot(grid):
    return torch.cat([(grid == (i + 1)).sum(dim=-1, keepdim=True) for i in range(4)], dim=-1)
    ...
    def get_qvals(self, state, use_zombienet=True):
        if type(state) is tuple:
            ...
            if self.use_zombienet:
                ...
                zombie_grid = torch.sum(zombie_grid, dim=1).view(-1, config.N_LANES)
            if self.use_gridnet:
                plant_grid = self.gridnet(plant_grid)
            state_t = torch.cat([plant_grid, zombie_grid, state_t[:,2 * self._grid_size:]], dim=1)
        else:
            state_t = torch.FloatTensor(state).to(device=self.device)
            zombie_grid = state_t[self._grid_size:(2 * self._grid_size)].reshape(-1, config.LANE_LENGTH)
            plant_grid = state_t[:self._grid_size]
            if self.use_zombienet:
                ...
                zombie_grid = torch.sum(zombie_grid, dim=1)
            if self.use_gridnet:
                plant_grid = self.gridnet(plant_grid)
...
class DQNAgent:
    ...
    def take_step(self, mode='train'):
        ...
        if mode == 'explore':
            ...
            self.step_count += 1
        # Handle Gymnasium new API which may return 5-tuple
        step_res = self.env.step(action)
        if isinstance(step_res, tuple):
            if len(step_res) == 5:
                obs_raw, r, terminated, truncated, info = step_res
                done = bool(terminated or truncated)
            elif len(step_res) == 4:
                obs_raw, r, done, info = step_res
            else:
